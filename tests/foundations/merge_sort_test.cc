...
#include <cassert>
#include <libcormen/foundations/merge_sort.hpp>
#include <vector>
...
int main() {
  ...
  foundation::chapter_two::merge_sort::apply(a);
  foundation::chapter_two::merge_sort::apply(b);
  foundation::chapter_two::merge_sort::apply(c);
  assert(exp == a);
  assert(exp_b == b);
  assert(exp_c == c);
  return 0;
